            ...
            <div class="flex items-start gap-4">
                ...
                <div class="block" style="width:100%" >
                    <div class="flex justify-between" >
                        <h2 class="text-xl font-bold text-gray-800 dark:text-gray-100">{ &props.headline }</h2>
                        ...
                        <div class="grid grid-cols-3 col-span-2 gap-4">
                            <div>
                                ...
                                <ul class="text-sm text-gray-600 dark:text-gray-400 list-disc pl-5">
                                    { for props.keywords.iter().map(|word| html! { <li>{ word }</li> }) }
                                </ul>
                            </div>
                            <div>
                                <h3 class="text-md font-semibold text-gray-700 dark:text-gray-300 mb-1">{"Features"}</h3>
                                <ul class="text-sm text-gray-600 dark:text-gray-400 list-disc pl-5">
                                    { for props.features.iter().map(|feature| html! { <li>{ feature }</li> }) }
                                </ul>
                            </div>
                            <div>
...
pub fn main() -> Html {
    ...
    html! {
        <div class="space-y-4 p-6 bg-gray-100 dark:bg-gray-900 min-h-screen">
            { for sample_data.iter().map(|data| html! { <FeatureCard ..data.clone() /> }) }
        </div>
    }
}
